...
const ReportsView = {
  ...
  applyFilters() {
    ...
    document.getElementById(
      "records-count"
    ).textContent = `${filteredRecords.length} registros`;
  ...
  displayFilteredRecords(records) {
    ...
    sortedRecords.forEach((record) => {
      ...
      row.innerHTML = `
                ...
                <td>${displayData}</td>
                <td>
                    <button class="btn btn-sm btn-outline-primary view-record" data-record-id="${
                      record.id
                    }">
                        Ver
                    </button>
                    <button class="btn btn-sm btn-outline-danger delete-record" data-record-id="${
                      record.id
                    }">
                        Eliminar
                    </button>
    ...
    recordsList.querySelectorAll(".delete-record").forEach((button) => {
      button.addEventListener("click", (e) => {
        const recordId = e.target.getAttribute("data-record-id");
  ...
  showRecordDetails(recordId) {
    ...
    const detailsHTML = `
            ...
            <div class="mb-3">
                <strong>Fecha y Hora:</strong> ${UIUtils.formatDate(
                  record.timestamp
                )}
            </div>
            <div class="mb-3">
  ...
  generateReport() {
    ...
    if (!fieldId) {
      UIUtils.showAlert(
        "Seleccione un campo para generar el reporte",
        "warning",
    ...
    const reportData = RecordModel.generateReport(
      fieldId,
      aggregation,
      filters
    );
  ...
  confirmDeleteRecord(recordId) {
    ...
    confirmMessage.textContent = `¿Está seguro de eliminar este registro de "${
      entity.name
    }" del ${UIUtils.formatDate(
      record.timestamp
    ...
    newConfirmBtn.addEventListener("click", () => {
      ...
      if (deleted) {
        ...
        // Recargar registros con los filtros actuales
        this.applyFilters();
